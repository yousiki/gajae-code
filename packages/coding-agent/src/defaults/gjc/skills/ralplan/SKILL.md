...
While ralplan is active it is a pre-approval planning phase: product-code mutation tools (`write`/`edit`/`ast_edit`) are blocked exactly like deep-interview. `bash` is deliberately not part of this planning-phase mutation guard, so agents must not rely on a shell-write block for product-code or `.gjc/**` safety; persist planning artifacts through the sanctioned `gjc ralplan --write` path instead of shell redirects. Prefer passing the `--artifact` markdown **inline** (the content string) so no scratch file is needed; this is mandatory for restricted role agents (see below). Only the leader, and only when an artifact is too large to pass inline, may stage it as a file in a system temp directory (`os.tmpdir()`/`$TMPDIR`, `/tmp`, `/var/tmp`) outside the project tree and pass that path — never write scratch files into the repo or `.gjc/`. Product code is mutated only after the plan is approved and execution begins.
