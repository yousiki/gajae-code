...
- `GJC_MUX_BACKEND` / `GJC_HERDR_COMMAND`
  - Experimental Herdr adapter selector for typed mux service development only. Team remains a tmux-backed runtime: do not start, document, or assume Herdr-backed workers from this skill, and do not treat `GJC_MUX_BACKEND=herdr` as a replacement for `GJC_TMUX_COMMAND` / `GJC_TEAM_TMUX_COMMAND`. The Herdr MVP is external-binary opt-in, not bundled or selected by default, and Coordinator/team/lifecycle/GC/harness/tmux-scroll flows stay unsupported until a later approved rollout gate.
