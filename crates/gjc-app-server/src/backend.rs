...
pub trait AgentBackend: Send + Sync {
    ...
    // -- state / reads --
    async fn get_state(&self, ctx: &BackendCallContext, include: serde_json::Value) -> Result<serde_json::Value>;
    async fn get_messages(&self, ctx: &BackendCallContext) -> Result<serde_json::Value>;

    // -- gjc management (mirrors old RPC catalog) --
    async fn set_model(&self, ctx: &BackendCallContext, provider: &str, model_id: &str) -> Result<serde_json::Value>;
    async fn compact(&self, ctx: &BackendCallContext, custom_instructions: Option<&str>) -> Result<serde_json::Value>;
    ...
    async fn exec(&self, ctx: &BackendCallContext, params: serde_json::Value) -> Result<serde_json::Value>;
...
pub trait BackendFactory: Send + Sync {
    async fn create_thread(&self, params: serde_json::Value) -> Result<(BackendHandleInfo, std::sync::Arc<dyn AgentBackend>)>;
    async fn resume_thread(&self, params: serde_json::Value) -> Result<(BackendHandleInfo, std::sync::Arc<dyn AgentBackend>)>;
    async fn fork_thread(&self, params: serde_json::Value) -> Result<(BackendHandleInfo, std::sync::Arc<dyn AgentBackend>)>;
}
...
mod tests {
    ...
    impl AgentBackend for FakeBackend {
        ...
        async fn set_model(&self, _c: &BackendCallContext, provider: &str, model_id: &str) -> Result<serde_json::Value> {
        ...
        async fn exec(&self, _c: &BackendCallContext, _p: serde_json::Value) -> Result<serde_json::Value> {
    ...
    async fn fake_backend_is_object_safe_and_drivable() {
        ...
        let turn = b.prompt(&ctx(), serde_json::json!({"input": "hi"})).await.unwrap();
