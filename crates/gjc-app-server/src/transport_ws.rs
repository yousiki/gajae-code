...
use crate::{
    ...
    jsonrpc::{Notification, RequestId, Response, parse_inbound},
    server::{AppServer, EventSink},
};
...
impl EventSink for WsEventSink {
    fn emit(&self, note: Notification) { let _ = self.tx.send(note); }
}
...
impl WsServerHandle {
    ...
    #[must_use]
    pub fn url(&self) -> String { format!("ws://{}:{}", self.addr.ip(), self.addr.port()) }

    pub async fn shutdown(&self) -> std::io::Result<()> {
        if self.stopped.swap(true, Ordering::SeqCst) { return Ok(()); }
        self.cancel.cancel();
...
pub async fn start_ws(core: Arc<AppServer>, config: WsServerConfig) -> std::io::Result<WsServerHandle> {
    let listener = TcpListener::bind(format!("{}:{}", config.host, config.port)).await?;
    let addr = listener.local_addr()?;
    let mut record = DiscoveryRecord::new(config.session_id.clone(), &config.host, addr.port(), config.token.clone());
    ...
    Ok(WsServerHandle { addr, session_id: config.session_id, state_root: config.state_root, cancel, accept_task, stopped: AtomicBool::new(false) })
}

async fn accept_loop(listener: TcpListener, core: Arc<AppServer>, token: String, cancel: CancellationToken) {
    loop {
        tokio::select! {
...
async fn handle_conn(stream: TcpStream, core: Arc<AppServer>, token: String, cancel: CancellationToken) {
    let auth = move |req: &Request, resp: HandshakeResponse| -> Result<HandshakeResponse, ErrorResponse> {
        if req.headers().contains_key("origin") { return Err(error_response(StatusCode::FORBIDDEN, "forbidden")); }
        if req.method() == "GET" && req.uri().path() == "/readyz" { return Err(error_response(StatusCode::OK, "ready")); }
        if token_from_query(req.uri().query()).is_some_and(|t| tokens_match(&t, &token)) { Ok(resp) } else { Err(error_response(StatusCode::UNAUTHORIZED, "unauthorized")) }
    };
    let Ok(ws) = accept_hdr_async(stream, auth).await else { return };
...
fn cleanup_discovery(state_root: &Path, session_id: &str) -> std::io::Result<()> {
    ...
    match std::fs::remove_file(&path) {
        ...
        Err(remove_err) => {
            ...
            let mut record: DiscoveryRecord = serde_json::from_str(&data).map_err(std::io::Error::other)?;
            record.stale = true;
            record.write_atomic(&path).map_err(|write_err| std::io::Error::new(write_err.kind(), format!("failed to remove discovery record ({remove_err}); also failed to mark stale: {write_err}")))
        }
    }
}
