...
pub fn parse_inbound(line: &str) -> crate::error::Result<Inbound> {
    let value: serde_json::Value = serde_json::from_str(line)
        .map_err(|e| crate::error::AppServerError::new(crate::error::codes::PARSE_ERROR, e.to_string()))?;
    let obj = value
        .as_object()
        .ok_or_else(|| crate::error::AppServerError::new(crate::error::codes::INVALID_REQUEST, "frame is not an object"))?;
    if obj.contains_key("id") {
        let req: Request = serde_json::from_value(value)
            .map_err(|e| crate::error::AppServerError::new(crate::error::codes::INVALID_REQUEST, e.to_string()))?;
        Ok(Inbound::Request(req))
    } else {
        let note: ClientNotification = serde_json::from_value(value)
            .map_err(|e| crate::error::AppServerError::new(crate::error::codes::INVALID_REQUEST, e.to_string()))?;
