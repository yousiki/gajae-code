...
impl ThreadStream {
    ...
    pub fn new(thread_id: ThreadId) -> Self {
        Self { thread_id, seq: SeqCounter::default(), active_turn: None, message_item: None, latch: TerminalLatch::new() }
    }
    ...
    fn raw(&mut self, ev: &BackendEvent) -> Notification {
        self.note(
            "gjc/event",
            serde_json::json!({ "eventType": ev.event_type, "event": ev.payload }),
        )
    }
    ...
    fn complete_message_item(&mut self, out: &mut Vec<Notification>) {
        if let Some(item) = self.message_item.take() {
            out.push(self.note("item/completed", serde_json::json!({ "itemId": item.0, "itemType": "agentMessage" })));
        }
    }
    ...
    fn flush_terminal(&mut self, out: &mut Vec<Notification>) {
        if let Some(cause) = self.latch.flush() {
            ...
            out.push(self.note("turn/completed", serde_json::json!({ "turnId": turn_id, "status": status })));
    ...
    pub fn on_event(&mut self, ev: &BackendEvent) -> Vec<Notification> {
        ...
        match ev.event_type.as_str() {
            ...
            "message_start" => {
                ...
                out.push(self.note("item/started", serde_json::json!({ "itemId": item.0, "itemType": "agentMessage" })));
            }
            ...
            "thinking_start" | "reasoning" => {
                ...
                if !ev.payload.get("redacted").and_then(|v| v.as_bool()).unwrap_or(false) {
            ...
            "tool_execution_start" => {
                let tool = ev.payload.get("toolName").and_then(|v| v.as_str()).unwrap_or("");
                let call_id = ev.payload.get("toolCallId").and_then(|v| v.as_str()).map(String::from);
            ...
            "tool_execution_end" => {
                let call_id = ev.payload.get("toolCallId").and_then(|v| v.as_str()).unwrap_or("");
                out.push(self.note(
                    "item/completed",
...
mod tests {
    ...
    fn redacted_reasoning_omits_content() {
        let mut s = ThreadStream::new(ThreadId("thr_1".into()));
        let out = s.on_event(&ev("thinking_start", serde_json::json!({"content": "secret", "redacted": true})));
