...
//! `orchestrator::drive_to_merge` performs after a run, exercised deterministically
//! (no engine/LLM). Skipped unless the env is set:
//!
//! ```sh
...
use git_daemon::forge_adapter::ForgeAdapter;
use git_daemon::github_forge::GithubForge;
use git_daemon::merge_gate::{GateInputs, evaluate};
...
async fn live_daemon_opens_and_merges_pr() {
	let (Ok(token), Ok(repo), Ok(base)) = (
		...
		eprintln!("SKIP live_daemon_opens_and_merges_pr: set GIT_DAEMON_GITHUB_TOKEN + GIT_DAEMON_REPO + GIT_DAEMON_DEV_BRANCHES");
		return;
	};
	...
	let pr = if let Some(pr) = ForgeAdapter::find_work_pr(&forge, &work_key).await.expect("find_work_pr") {
		...
		let pr = ForgeAdapter::create_pr(&forge, &head, &base, "git-daemon: live e2e merge", "Autonomous merge of the agent-authored fix.").await.expect("create_pr");
	...
	// 4. SHA-bound merge.
	let merge_sha = ForgeAdapter::merge_pr(&forge, &git_daemon::forge_adapter::MergeRequest { pr_id: pr_ref, expected_head_sha: decision.head_sha }).await.expect("merge_pr");
